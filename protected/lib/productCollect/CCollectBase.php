...
//message('下载图片:'.$links);
        if (!file_exists($savename) || filesize($savename) <= 0) {
            ...
            if (curl_errno($ch)) {
                $this->trace('img curl:' . curl_error($ch));
                return false;
            }
            curl_close($ch);
            if ($img_data !== false) {
    ...
    protected function getInterHtmlDom($url, $cookie = '') {
        ...
        $this->createPhpQuery($this->content);

        return true;
    }

    protected function createPhpQuery($content) {
        $this->clear();
        ...
        try {
            ...
            if ($this->info['stock'] != 0) {
                ...
                if ($this->getInfo('brandName'))
                    $product->brandName = $this->info['brandName'];

                if ($this->getInfo('productTitle'))
                    $product->productTitle = $this->info['productTitle'];
                ...
                if ($this->getInfo('tags'))
                    $product->tags = $this->info['tags'];
